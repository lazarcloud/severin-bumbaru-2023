...
export async function chart_trainROUTE(dep_place, dep_time, cities){
        ...
        traintimes.push([ cityroute[dep_place], cityroute[1], get_time(dep_place, route[0][0]) * 60 ]);
        for(let i = 1; i < cityroute.length - 1; i ++){
            traintimes.push([cityroute[i], cityroute[i + 1], get_time(route[i - 1][0], route[i][0]) * 60 ]);
            staytimes.push([cityroute[i], route[i][1]]);
        }
        traintimes.push([cityroute[route.length], cityroute[dep_place], get_time(route[route.length - 1][0], dep_place) * 60 ]);
